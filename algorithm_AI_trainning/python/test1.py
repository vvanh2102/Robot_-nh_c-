...
class GomokuHeuristic:
    ...
    def __init__(self, board, size):
        self.board = board  # Bàn cờ (dictionary lưu trữ các ô)
    ...
    def row_to_list(self, y, x, dy, dx, yf, xf):
        ...
        while y != yf + dy or x != xf + dx:  # Lặp cho đến khi đến vị trí cuối cùng
            row.append(self.board.get((y, x), ' '))  # Thêm ô vào danh sách
...
class GomokuGame:
    ...
    def is_winner(self, player):
        for y in range(self.size):
            for x in range(self.size - 4):
                if all(self.board.get((y, x + i), ' ') == player for i in range(5)):  # Kiểm tra hàng ngang
                    return True
                if all(self.board.get((x + i, y), ' ') == player for i in range(5)):  # Kiểm tra hàng dọc
                    return True
        for y in range(self.size - 4):
            for x in range(self.size - 4):
                if all(self.board.get((y + i, x + i), ' ') == player for i in range(5)):  # Kiểm tra đường chéo chính
                    return True
                if all(self.board.get((y + i, x + 4 - i), ' ') == player for i in range(5)):  # Kiểm tra đường chéo phụ
                    return True
        return False
    ...
    def get_top_moves(self, col, anticol, top_n=5):
        ...
        for y in range(self.size):
            for x in range(self.size):
                if self.board.get((y, x), ' ') == ' ':  # Chỉ xét các ô trống
