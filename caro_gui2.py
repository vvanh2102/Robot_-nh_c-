...
# PyQt6 imports
os.environ['QT_QPA_PLATFORM'] = 'windows'  # Fix Qt platform plugin warning - force X11
...
class DetectionWorker(QThread):
    # V2 – ổn định mạnh, khóa ROI, chống double-detection
    ...
    def __init__(self, conf_piece=CONF_PIECE, shrink=SHRINK_FACTOR, parent=None):
        ...
        # ---------- NEW: khóa ROI ----------
        self.fixed_roi = None
        self.enable_roi_lock = True

        # ---------- NEW: ổn định detection ----------
        self.history = []
        self.max_history = 10   # sử dụng 15 frame
    ...
    def _ensure_models(self) -> bool:
        if not _YOLO_AVAILABLE:
            self.ready.emit(False, "YOLO chưa cài đặt.")
            return False
    ...
    def _find_board_roi(self, frame):
        ...
        roi = BoardROI(x1, y1, x2, y2)

        # ---------- NEW: khóa ROI ----------
        if self.enable_roi_lock:
            self.fixed_roi = roi
    ...
    # ------------------------------
    def _merge_close_points(self, points, threshold=8):
        if not points:
            return points
        ...
        for i, p in enumerate(points):
            ...
            r1, c1, cls1, u1, v1 = p
            cluster = [(r1, c1, cls1, u1, v1)]
            used.add(i)

            for j, q in enumerate(points):
                if j in used: continue
                r2, c2, cls2, u2, v2 = q
                if ((u1 - u2)**2 + (v1 - v2)**2)**0.5 < threshold:
                    cluster.append(q)
                    used.add(j)

            # lấy điểm đại diện trung bình
            avg = np.mean([[p[3], p[4]] for p in cluster], axis=0)
            u_avg, v_avg = int(avg[0]), int(avg[1])

            # convert lại row/col theo cluster head
            row, col, cls_id, _, _ = cluster[0]
            merged.append((row, col, cls_id, u_avg, v_avg))
    ...
    def _temporal_filter(self, detections):
        ...
        # các detection xuất hiện trong tất cả frame gần đây
        stable = {}
        for frame in self.history:
            for item in set((r, c, cls) for r, c, cls, _, _ in frame):
                stable[item] = stable.get(item, 0) + 1

        stable_items = {k for k,v in stable.items() if v >= self.max_history}

        return [d for d in detections if (d[0], d[1], d[2]) in stable_items]
    ...
    def _detect_frame(self, frame):
        ...
        items = []
        if roi is None:
            cv2.putText(annotated, "NO BOARD", (30,40), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,255),2)
        ...
        result = self.piece_model(crop, conf=self.conf_piece, verbose=False)[0]
        ...
        def clamp(v): return int(max(0, min(GRID_SIZE-1, int(round(v)))))

        # convert YOLO to grid
        for idx, (x1,y1,x2,y2) in enumerate(boxes):
            cx = (x1+x2)/2
            cy = (y1+y2)/2
            abs_u = int(roi.x1 + cx)
            abs_v = int(roi.y1 + cy)
            col = clamp((abs_u - roi.x1) / step_x)
            row = clamp((abs_v - roi.y1) / step_y)
            cls_id = 1 if classes[idx]==0 else 2
            raw.append((row, col, cls_id, abs_u, abs_v))

        # ---------- NEW: gộp detection gần nhau ----------
        merged = self._merge_close_points(raw)

        # ---------- NEW: lọc ổn định ----------
        stable = self._temporal_filter(merged)

        # vẽ kết quả
        for (row, col, cls_id, u, v) in stable:
            color = (0,255,0) if cls_id==1 else (0,255,255)
            cv2.circle(annotated, (u,v), 6, color, -1)
            cv2.putText(annotated, f"{row},{col}", (u+6, v-6),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1)
            items.append(DetectionItem(row, col, cls_id, u, v))
...
class MainWindow(QMainWindow):
    ...
    def on_det_result(self, annotated: np.ndarray, roi: Optional[BoardROI], items: List[DetectionItem]) -> None:
        ...
        if self.auto_play_mode and self.game_active:
            # --- CODE MỚI BẮT ĐẦU ---
            # 1. Lấy số lượng quân cờ hiện tại đang có trong bộ nhớ Game State
            current_board = self.game_state.get_board_state()
            known_piece_count = np.count_nonzero(current_board)

            # 2. Đếm số lượng quân cờ vừa nhìn thấy qua Camera
            detected_piece_count = len(items)

            # 3. Logic chặn lỗi:
            # Nếu Camera nhìn thấy ít hơn số quân đang có (nghĩa là đang bị tay che)
            # -> Bỏ qua, không xử lý để tránh AI hiểu nhầm là mất quân
            if detected_piece_count < known_piece_count:
                # Có thể in log debug nếu muốn: print("Hand obstructing view, skipping...")
                return

                # Nếu số lượng nhìn thấy chênh lệch quá nhiều (ví dụ > 2 quân so với bộ nhớ)
            # -> Cũng bỏ qua luôn cho an toàn (tránh trường hợp tay vừa rút ra chưa hết)
            if detected_piece_count > known_piece_count + 1:
                return
            # --- CODE MỚI KẾT THÚC ---
