...
class RobotClient:
    ...
    def pick_from_reserve(self, dwell=1):
        p = "D:/doancaro/calibration/reserve_point.npz"
        ...
        self.rb.move_cartesian(X=Xr, Y=Yr, Z=Z_PK, A=Ar, B=Br, C=Cr,
                               ...
                               wait_move_finished=True)
        self.vacuum_on(); time.sleep(dwell)
        # nhấc lên về Z_APP
        self.rb.move_cartesian(X=Xr, Y=Yr, Z=Z_APP, A=Ar, B=Br, C=Cr,
                               E1=0.0, E2=0.0, E3=0.0,
                               velocity=self.v_approach, frame="#base",
...
def pick_from_reserve(self, dwell=0.3):
    """Di chuyển tới vị trí khay quân, hút và nhấc lên."""
    import numpy as np
    path = "D:/doancaro/calibration/reserve_point.npz"
    if not os.path.exists(path):
        raise FileNotFoundError("Chưa có reserve_point.npz — hãy teach trước.")
    d = np.load(path)
    Xr,Yr,Zr = float(d["X"]), float(d["Y"]), float(d["Z"])
    Ar,Br,Cr = float(d["A"]), float(d["B"]), float(d["C"])
    ...
    print(f"[RESERVE] Lấy quân tại ({Xr:.1f},{Yr:.1f},{Zr:.1f})")
    self.rb.move_cartesian(X=Xr, Y=Yr, Z=Z_APPROACH, A=Ar, B=Br, C=Cr,
                            velocity=self.v_approach, frame="#base",
                            wait_move_finished=True)
    self.rb.move_cartesian(X=Xr, Y=Yr, Z=Z_PICK, A=Ar, B=Br, C=Cr,
                            velocity=self.v_touch, frame="#base",
                            wait_move_finished=True)
    self.vacuum_on();  time.sleep(dwell)
    self.lift_up(Xr, Yr)
